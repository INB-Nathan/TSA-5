...
<head>
    <meta charset="UTF-8">
    <title>Brewkaholic - Manage Users</title>
...
<body class="font-serif bg-bg-primary text-text-primary leading-relaxed">
    ...
    <div class="max-w-[1200px] mx-auto py-12 px-8">
        <h1 class="text-4xl font-bold tracking-[3px] mb-8 text-accent">MANAGE USERS</h1>

        <?php if (session()->getFlashdata('msg')) : ?>
            <div class="bg-[#3a2a1a] border border-accent text-accent p-4 mb-8 text-center"><?= session()->getFlashdata('msg') ?></div>
        <?php endif; ?>

        <table class="w-full border-collapse bg-bg-secondary border border-border-color md:text-sm">
            <thead>
                <tr>
                    <th class="p-4 text-left border-b border-border-color bg-bg-primary text-accent font-bold tracking-[1px] md:p-3 md:px-2">ID</th>
                    <th class="p-4 text-left border-b border-border-color bg-bg-primary text-accent font-bold tracking-[1px] md:p-3 md:px-2">Username</th>
                    <th class="p-4 text-left border-b border-border-color bg-bg-primary text-accent font-bold tracking-[1px] md:p-3 md:px-2">Email</th>
                    <th class="p-4 text-left border-b border-border-color bg-bg-primary text-accent font-bold tracking-[1px] md:p-3 md:px-2">Role</th>
                    <th class="p-4 text-left border-b border-border-color bg-bg-primary text-accent font-bold tracking-[1px] md:p-3 md:px-2">Created At</th>
                    <th class="p-4 text-left border-b border-border-color bg-bg-primary text-accent font-bold tracking-[1px] md:p-3 md:px-2">Actions</th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($users)) : ?>
                ...
                <?php else : ?>
                    <?php foreach ($users as $user) : ?>
                        <tr class="hover:bg-bg-tertiary">
                            <td class="p-4 text-left border-b border-border-color text-text-secondary md:p-3 md:px-2"><?= $user->id ?></td>
                            <td class="p-4 text-left border-b border-border-color text-text-secondary md:p-3 md:px-2"><?= esc($user->username) ?></td>
                            <td class="p-4 text-left border-b border-border-color text-text-secondary md:p-3 md:px-2"><?= esc($user->email) ?></td>
                            <td class="p-4 text-left border-b border-border-color text-text-secondary md:p-3 md:px-2"><?= esc($user->role_name ?? 'No role') ?></td>
                            <td class="p-4 text-left border-b border-border-color text-text-secondary md:p-3 md:px-2"><?= date('Y-m-d H:i', strtotime($user->created_at)) ?></td>
                            <td class="p-4 text-left border-b border-border-color text-text-secondary md:p-3 md:px-2">
        ...
        <?php if (isset($totalUsers) && $totalUsers > 0 && isset($totalPages)) : ?>
            <div class="flex justify-center gap-2 mt-8 flex-wrap">
                <?php if ($currentPage > 1) : ?>
                    <a href="/admin/users?page=<?= $currentPage - 1 ?>" class="px-4 py-2 border border-accent text-accent no-underline transition-all hover:bg-accent hover:text-bg-primary md:px-3 md:py-1.5 md:text-xs">Previous</a>
                <?php else : ?>
                ...
                if ($startPage > 1) : ?>
                    <a href="/admin/users?page=1" class="px-4 py-2 border border-accent text-accent no-underline transition-all hover:bg-accent hover:text-bg-primary md:px-3 md:py-1.5 md:text-xs">1</a>
                    <?php if ($startPage > 2) : ?>
                        <span>...</span>
                    <?php endif; ?>
                <?php endif; ?>
                ...
                <?php for ($i = $startPage; $i <= $endPage; $i++) : ?>
                    ...
                    <?php else : ?>
                        <a href="/admin/users?page=<?= $i ?>" class="px-4 py-2 border border-accent text-accent no-underline transition-all hover:bg-accent hover:text-bg-primary md:px-3 md:py-1.5 md:text-xs"><?= $i ?></a>
                    <?php endif; ?>
                <?php endfor; ?>
                ...
                <?php if ($endPage < $totalPages) : ?>
                    <?php if ($endPage < $totalPages - 1) : ?>
                        <span>...</span>
                    <?php endif; ?>
                    <a href="/admin/users?page=<?= $totalPages ?>" class="px-4 py-2 border border-accent text-accent no-underline transition-all hover:bg-accent hover:text-bg-primary md:px-3 md:py-1.5 md:text-xs"><?= $totalPages ?></a>
                <?php endif; ?>
                
                <?php if ($currentPage < $totalPages) : ?>
                    <a href="/admin/users?page=<?= $currentPage + 1 ?>" class="px-4 py-2 border border-accent text-accent no-underline transition-all hover:bg-accent hover:text-bg-primary md:px-3 md:py-1.5 md:text-xs">Next</a>
                <?php else : ?>
